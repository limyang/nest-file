...
public final class OrcFile {
	 ...
	 * To make it easier for users to understand the version numbers, we use the Hive release number that first wrote
	 * that version of ORC files.
	 * 
	 * Thus, if you add new encodings or other non-forward compatible changes to ORC files, which prevent the old reader
	 * from reading the new format, you should change these variable to reflect the next Hive release number.
	 * Non-forward compatible changes should never be added in patch releases.
	 * 
	 * Do not make any changes that break backwards compatibility, which would prevent the new reader from reading ORC
	 * files generated by any released version of Hive.
	 */
	...
	static final long DEFAULT_STRIPE_SIZE = 256L * 1024 * 1024;
