...
class WriterImpl implements Writer, MemoryManager.Callback {
	...
	WriterImpl(FileSystem fs, Path path, Configuration conf, ObjectInspector inspector, long stripeSize,
			...
			boolean addBlockPadding, OrcFile.Version version) throws IOException {
		this.fs = fs;
		this.path = path;
		this.conf = conf;
	...
	/**
	 * Interface from the Writer to the TreeWriters. This limits the visibility that the TreeWriters have into the
	 * Writer.
	 */
	...
	/**
	 * The parent class of all of the writers for each column. Each column is written by an instance of this class. The
	 * compound types (struct, list, map, and union) have children tree writers that write the children types.
	 */
	private abstract static class TreeWriter {
		 ...
		 * 
		 * @param builder
		 *            the stripe footer that contains the information about the layout of the stripe. The TreeWriter is
		 *            required to update the footer with its information.
		 * @param requiredIndexEntries
		 *            the number of index entries that are required. this is to check to make sure the row index is well
		 *            formed.
		 * @throws IOException
		 */
			...
			if (rowIndexStream != null) {
				if (rowIndex.getEntryCount() != requiredIndexEntries) {
					throw new IllegalArgumentException("Column has wrong number of " + "index entries found: "
							+ rowIndexEntry + " expected: " + requiredIndexEntries);
				}
		...
		/**
		 * Create a row index entry with the previous location and the current index statistics. Also merges the index
		 * statistics into the file statistics before they are cleared. Finally, it records the start of the next index
	...
	private static class IntegerTreeWriter extends TreeWriter {
		...
		IntegerTreeWriter(int columnId, ObjectInspector inspector, StreamFactory writer, boolean nullable)
				throws IOException {
			super(columnId, inspector, writer, nullable);
			PositionedOutputStream out = writer.createStream(id, OrcProto.Stream.Kind.DATA);
	...
	private static class StringTreeWriter extends TreeWriter {
		...
		private final StringRedBlackTree dictionary = new StringRedBlackTree(INITIAL_DICTIONARY_SIZE);
		...
		StringTreeWriter(int columnId, ObjectInspector inspector, StreamFactory writer, boolean nullable)
				throws IOException {
			super(columnId, inspector, writer, nullable);
			this.isDirectV2 = isNewWriteFormat(writer);
		...
		/**
		 * Method to retrieve string values from the value object, which can be overridden by subclasses.
		 * 
		 * @param obj
		 *            value
		...
		void writeStripe(OrcProto.StripeFooter.Builder builder, int requiredIndexEntries) throws IOException {
			...
			useDictionaryEncoding = (!isDirectV2)
		...
		/**
		 * This method doesn't call the super method, because unlike most of the other TreeWriters, this one can't
		 * record the position in the streams until the stripe is being flushed. Therefore it saves all of the entries
	...
	private static class VarcharTreeWriter extends StringTreeWriter {
		...
		VarcharTreeWriter(int columnId, ObjectInspector inspector, StreamFactory writer, boolean nullable)
				throws IOException {
			super(columnId, inspector, writer, nullable);
		}
		...
		@Override
		String getStringValue(Object obj) {
			return (((HiveVarcharObjectInspector) inspector).getPrimitiveJavaObject(obj)).getValue();
		}
	}
	...
	private static class BinaryTreeWriter extends TreeWriter {
		...
		BinaryTreeWriter(int columnId, ObjectInspector inspector, StreamFactory writer, boolean nullable)
			...
			this.stream = writer.createStream(id, OrcProto.Stream.Kind.DATA);
			this.isDirectV2 = isNewWriteFormat(writer);
			this.length = createIntegerWriter(writer.createStream(id, OrcProto.Stream.Kind.LENGTH), false, isDirectV2);
			recordPosition(rowIndexPosition);
		}
	...
	static final long BASE_TIMESTAMP = Timestamp.valueOf("2015-01-01 00:00:00").getTime() / MILLIS_PER_SECOND;
	...
	private static class TimestampTreeWriter extends TreeWriter {
		...
		TimestampTreeWriter(int columnId, ObjectInspector inspector, StreamFactory writer, boolean nullable)
			...
			this.nanos = createIntegerWriter(writer.createStream(id, OrcProto.Stream.Kind.SECONDARY), false, isDirectV2);
			recordPosition(rowIndexPosition);
		}
	...
	private static class DateTreeWriter extends TreeWriter {
		...
		DateTreeWriter(int columnId, ObjectInspector inspector, StreamFactory writer, boolean nullable)
				throws IOException {
			super(columnId, inspector, writer, nullable);
			PositionedOutputStream out = writer.createStream(id, OrcProto.Stream.Kind.DATA);
			this.isDirectV2 = isNewWriteFormat(writer);
	...
	private static class DecimalTreeWriter extends TreeWriter {
		...
		DecimalTreeWriter(int columnId, ObjectInspector inspector, StreamFactory writer, boolean nullable)
				throws IOException {
			super(columnId, inspector, writer, nullable);
			this.isDirectV2 = isNewWriteFormat(writer);
			valueStream = writer.createStream(id, OrcProto.Stream.Kind.DATA);
			this.scaleStream = createIntegerWriter(writer.createStream(id, OrcProto.Stream.Kind.SECONDARY), true,
					isDirectV2);
			recordPosition(rowIndexPosition);
		}
	...
	private static class StructTreeWriter extends TreeWriter {
		...
		StructTreeWriter(int columnId, ObjectInspector inspector, StreamFactory writer, boolean nullable)
			...
			for (int i = 0; i < childrenWriters.length; ++i) {
				childrenWriters[i] = createTreeWriter(fields.get(i).getFieldObjectInspector(), writer, true);
			}
			recordPosition(rowIndexPosition);
		}
	...
	private static class ListTreeWriter extends TreeWriter {
		...
		ListTreeWriter(int columnId, ObjectInspector inspector, StreamFactory writer, boolean nullable)
			...
			lengths = createIntegerWriter(writer.createStream(columnId, OrcProto.Stream.Kind.LENGTH), false, isDirectV2);
			recordPosition(rowIndexPosition);
		}
	...
	private static class MapTreeWriter extends TreeWriter {
		...
		MapTreeWriter(int columnId, ObjectInspector inspector, StreamFactory writer, boolean nullable)
			...
			childrenWriters[0] = createTreeWriter(insp.getMapKeyObjectInspector(), writer, true);
			childrenWriters[1] = createTreeWriter(insp.getMapValueObjectInspector(), writer, true);
			lengths = createIntegerWriter(writer.createStream(columnId, OrcProto.Stream.Kind.LENGTH), false, isDirectV2);
			recordPosition(rowIndexPosition);
		}
	...
	private static TreeWriter createTreeWriter(ObjectInspector inspector, StreamFactory streamFactory, boolean nullable)
			throws IOException {
		...
		case PRIMITIVE:
			...
			case VARCHAR:
				...
				VarcharTypeParams varcharParams = (VarcharTypeParams) ParameterizedPrimitiveTypeUtils
						.getTypeParamsFromPrimitiveObjectInspector((PrimitiveObjectInspector) treeWriter.inspector);
	...
	private void ensureWriter() throws IOException {
		if (rawWriter == null) {
			rawWriter = fs.create(path, false, HDFS_BUFFER_SIZE, fs.getDefaultReplication(), blockSize);
	...
	private int writeFooter(long bodyLength) throws IOException {
		...
		for (Map.Entry<String, ByteString> entry : userMetadata.entrySet()) {
			builder.addMetadata(OrcProto.UserMetadataItem.newBuilder().setName(entry.getKey())
	...
	private int writePostScript(int footerLength) throws IOException {
		OrcProto.PostScript.Builder builder = OrcProto.PostScript.newBuilder()
				.setCompression(writeCompressionKind(compress)).setFooterLength(footerLength).setMagic(OrcFile.MAGIC)
				.addVersion(version.getMajor()).addVersion(version.getMinor());
