...
class RecordReaderImpl implements RecordReader {
	...
	RecordReaderImpl(Iterable<StripeInformation> stripes, FileSystem fileSystem, Path path, long offset, long length,
			List<OrcProto.Type> types, CompressionCodec codec, int bufferSize, boolean[] included, long strideRate,
			SearchArgument sarg, String[] columnNames) throws IOException {
	...
	private abstract static class TreeReader {
		...
		void checkEncoding(OrcProto.ColumnEncoding encoding) throws IOException {
			if (encoding.getKind() != OrcProto.ColumnEncoding.Kind.DIRECT) {
				throw new IOException("Unknown encoding " + encoding + " in column " + columnId + " of " + path);
			}
		}

		IntegerReader createIntegerReader(OrcProto.ColumnEncoding.Kind kind, InStream in, boolean signed)
				throws IOException {
			switch (kind) {
			case DIRECT_V2:
			case DICTIONARY_V2:
	...
	private static class TimestampTreeReader extends TreeReader {
		...
		void startStripe(Map<StreamName, InStream> streams, List<OrcProto.ColumnEncoding> encodings) throws IOException {
			...
			nanos = createIntegerReader(encodings.get(columnId).getKind(),
					streams.get(new StreamName(columnId, OrcProto.Stream.Kind.SECONDARY)), false);
		}
	...
	private static class DecimalTreeReader extends TreeReader {
		...
		void startStripe(Map<StreamName, InStream> streams, List<OrcProto.ColumnEncoding> encodings) throws IOException {
			super.startStripe(streams, encodings);
			valueStream = streams.get(new StreamName(columnId, OrcProto.Stream.Kind.DATA));
			scaleStream = createIntegerReader(encodings.get(columnId).getKind(),
		...
		Object next(Object previous) throws IOException {
			super.next(previous);
			if (valuePresent) {
				return new HiveDecimal(SerializationUtils.readBigInteger(valueStream), (int) scaleStream.next());
			}
			return null;
		}
	...
	/**
	 * A tree reader that will read string columns. At the start of the stripe, it creates an internal reader based on
	 * whether a direct or dictionary encoding was used.
	 */
	...
	private static class ListTreeReader extends TreeReader {
		...
		ListTreeReader(Path path, int columnId, List<OrcProto.Type> types, boolean[] included) throws IOException {
			super(path, columnId);
			OrcProto.Type type = types.get(columnId);
			elementReader = createTreeReader(path, type.getSubtypes(0), types, included);
		}
	...
	private static TreeReader createTreeReader(Path path, int columnId, List<OrcProto.Type> types, boolean[] included)
			throws IOException {
		OrcProto.Type type = types.get(columnId);
		switch (type.getKind()) {
		case BOOLEAN:
	...
	/**
	 * Given a point and min and max, determine if the point is before, at the min, in the middle, at the max, or after
	 * the range.
	 * 
	 * @param point
	 *            the point to test
	 ...
	 * 
	 * @param index
	 *            the statistics for the column mentioned in the predicate
	 * @param predicate
	 *            the leaf predicate we need to evaluation
	 * @return the set of truth values that may be returned for the given predicate.
	 */
	static TruthValue evaluatePredicate(OrcProto.ColumnStatistics index, PredicateLeaf predicate) {
		...
		case NULL_SAFE_EQUALS:
			loc = compareToRange((Comparable) predicate.getLiteral(), minValue, maxValue);
		...
		case BETWEEN:
			...
			if (loc == Location.BEFORE || loc == Location.MIN) {
				Location loc2 = compareToRange((Comparable) args.get(1), minValue, maxValue);
	 ...
	 * 
	 * @return an array with a boolean for each row group or null if all of the row groups must be read.
	 * @throws IOException
	 */
	private boolean[] pickRowGroups() throws IOException {
		...
		for (int rowGroup = 0; rowGroup < result.length; ++rowGroup) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Row group " + (rowIndexStride * rowGroup) + " to " + (rowIndexStride * (rowGroup + 1) - 1)
						+ " is " + (result[rowGroup] ? "" : "not ") + "included.");
			}
		}
	...
	private void readAllDataStreams(StripeInformation stripe) throws IOException {
		...
		for (OrcProto.Stream section : stripeFooter.getStreamsList()) {
			if (StreamName.getArea(section.getKind()) == StreamName.Area.DATA) {
				...
				streams.put(name, InStream.create(name.toString(), new ByteBuffer[] { sectionBuffer },
	 ...
	 */
	static int getIndexPosition(OrcProto.ColumnEncoding.Kind encoding, OrcProto.Type.Kind type,
			OrcProto.Stream.Kind stream, boolean isCompressed, boolean hasNulls) {
	...
	static boolean isDictionary(OrcProto.Stream.Kind kind, OrcProto.ColumnEncoding encoding) {
		...
		return kind == OrcProto.Stream.Kind.DICTIONARY_DATA
				|| (kind == OrcProto.Stream.Kind.LENGTH && (encodingKind == OrcProto.ColumnEncoding.Kind.DICTIONARY || encodingKind == OrcProto.ColumnEncoding.Kind.DICTIONARY_V2));
	}

	/**
	 * Plan the ranges of the file that we need to read given the list of columns and row groups.
	 * 
	 * @param streamList
	 *            the list of streams avaiable
	 * @param indexes
	 *            the indexes that have been loaded
	 * @param includedColumns
	 *            which columns are needed
	 * @param includedRowGroups
	 *            which row groups are needed
	 * @param isCompressed
	 ...
	 */
	static List<DiskRange> planReadPartialDataStreams(List<OrcProto.Stream> streamList, OrcProto.RowIndex[] indexes,
			boolean[] includedColumns, boolean[] includedRowGroups, boolean isCompressed,
			List<OrcProto.ColumnEncoding> encodings, List<OrcProto.Type> types, int compressionSize) {
				...
				if (includedRowGroups == null || isDictionary(streamKind, encodings.get(column))) {
					...
					for (int group = 0; group < includedRowGroups.length; ++group) {
						if (includedRowGroups[group]) {
							int posn = getIndexPosition(encodings.get(column).getKind(), types.get(column).getKind(),
									stream.getKind(), isCompressed, hasNull[column]);
							...
							long end = (group == includedRowGroups.length - 1) ? length : Math.min(length,
									indexes[column].getEntry(group + 1).getPositions(posn)
											+ (isCompressed ? (OutStream.HEADER_SIZE + compressionSize)
	...
	/**
	 * Update the disk ranges to collapse adjacent or overlapping ranges. It assumes that the ranges are sorted.
	 * 
	 * @param ranges
	 *            the list of disk ranges to merge
	 */
	 ...
	 * @param ranges
	 *            the disk ranges within the stripe to read
	...
	static void createStreams(List<OrcProto.Stream> streamDescriptions, List<DiskRange> ranges, byte[][] bytes,
			boolean[] includeColumn, CompressionCodec codec, int bufferSize, Map<StreamName, InStream> streams)
			throws IOException {
		long offset = 0;
		for (OrcProto.Stream streamDesc : streamDescriptions) {
			...
			if (includeColumn[column] && StreamName.getArea(streamDesc.getKind()) == StreamName.Area.DATA) {
				...
				for (int i = 0; i < buffers.length; ++i) {
					...
					buffers[i] = ByteBuffer.wrap(bytes[first + i], Math.max(0, (int) (offset - range.offset)),
							(int) (end - start));
	...
	private void readPartialDataStreams(StripeInformation stripe) throws IOException {
		...
		List<DiskRange> chunks = planReadPartialDataStreams(streamList, indexes, included, includedRowGroups,
				codec != null, stripeFooter.getColumnsList(), types, bufferSize);
		if (LOG.isDebugEnabled()) {
		...
		createStreams(streamList, chunks, bytes, included, codec, bufferSize, streams);
	}
			...
			if (stream.getKind() == OrcProto.Stream.Kind.ROW_INDEX) {
				...
				if ((included == null || included[col]) && indexes[col] == null) {
					...
					indexes[col] = OrcProto.RowIndex.parseFrom(InStream.create("index",
							new ByteBuffer[] { ByteBuffer.wrap(buffer) }, new long[] { 0 }, stream.getLength(), codec,
							bufferSize));
				}
			}
