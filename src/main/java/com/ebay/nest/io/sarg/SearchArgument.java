...
/**
 * Primary interface for <a href="http://en.wikipedia.org/wiki/Sargable"> SearchArgument</a>, which are the subset of
 * predicates that can be pushed down to the RecordReader. Each SearchArgument consists of a series of SearchClauses
 * that must each be true for the row to be accepted by the filter.
 * 
 * This requires that the filter be normalized into conjunctive normal form (<a
 * href="http://en.wikipedia.org/wiki/Conjunctive_normal_form">CNF</a>).
 */
public interface SearchArgument {
	 ...
	 * 
	 * @param leaves
	 *            the value of each leaf predicate
	...
	/**
	 * A builder object for contexts outside of Hive where it isn't easy to get a ExprNodeDesc. The user must call
	 * startOr, startAnd, or startNot before adding any leaves.
	 */
	public interface Builder {
		...
		/**
		 * Build and return the SearchArgument that has been defined. All of the starts must have been ended before this
		 * call.
		 * 
