...
import com.ebay.nest.io.sede.lazybinary.LazyBinaryUtils.RecordInfo;
 ...
 * 
 * Section A is the null-bytes. Suppose the map has N key-value pairs, then there are (N*2+7)/8 bytes used as
 * null-bytes. Each bit corresponds to a key or a value and it indicates whether that key or value is null (0) or not
 * null (1).
 * 
 * After A, all the bytes are actual serialized data of the map, which are key-value pairs. b represent the keys and c
 * represent the values. Each of them is again a LazyBinaryObject.
 * 
 */
...
public class LazyBinaryMap extends LazyBinaryNonPrimitive<LazyBinaryMapObjectInspector> {
	...
	/**
	 * The size of the map. Only valid when the data is parsed. -1 when the map is NULL.
	 */
	int mapSize = 0;

	/**
	 * The beginning position and length of key[i] and value[i]. Only valid when the data is parsed.
	 */
	int[] keyStart;
	int[] keyLength;
	int[] valueStart;
	...
	/**
	 * Whether valueObjects[i]/keyObjects[i] is null or not This could not be inferred from the length of the object. In
	 * particular, a 0-length string is not null.
	 */
	boolean[] keyIsNull;
	boolean[] valueIsNull;
	...
	/**
	 * The values are stored in an array of LazyObjects. value[index] will start from KeyEnd[index] + 1, and ends before
	 * KeyStart[index+1] - 1.
	 */
	...
	/**
	 * Adjust the size of arrays: keyStart, keyLength valueStart, valueLength keyInited, keyIsNull valueInited,
	 * valueIsNull.
	 */
	...
	/**
	 * Parse the byte[] and fill keyStart, keyLength, keyIsNull valueStart, valueLength and valueIsNull.
	 */
	private void parse() {
		...
		for (int i = 0; i < mapSize; i++) {
			...
			if ((bytes[nullByteCur] & (1 << ((i * 2) % 8))) != 0) {
				keyIsNull[i] = false;
				LazyBinaryUtils.checkObjectByteInfo(((MapObjectInspector) oi).getMapKeyObjectInspector(), bytes,
			...
			if ((bytes[nullByteCur] & (1 << ((i * 2 + 1) % 8))) != 0) {
				valueIsNull[i] = false;
				LazyBinaryUtils.checkObjectByteInfo(((MapObjectInspector) oi).getMapValueObjectInspector(), bytes,
						lastElementByteEnd, recordInfo);
	...
	private LazyBinaryObject uncheckedGetValue(int index) {
		...
		if (!valueInited[index]) {
			...
			if (valueObjects[index] == null) {
				valueObjects[index] = LazyBinaryFactory.createLazyBinaryObject(((MapObjectInspector) oi)
	 ...
	 * 
	 * If there are multiple matches (which is possible in the serialized format), only the first one is returned.
	 * 
	 * The most efficient way to get the value for the key is to serialize the key and then try to find it in the array.
	 * We do linear search because in most cases, user only wants to get one or two values out of the map, and the cost
	 * of building up a HashMap is substantially higher.
	 * 
	 * @param key
	...
	/**
	 * cachedMap is reused for different calls to getMap(). But each LazyBinaryMap has a separate cachedMap so we won't
	 * overwrite the data by accident.
	 */
	LinkedHashMap<Object, Object> cachedMap;

	/**
	 * Return the map object representing this LazyBinaryMap. Note that the keyObjects will be Writable primitive
	 * objects.
	 * 
