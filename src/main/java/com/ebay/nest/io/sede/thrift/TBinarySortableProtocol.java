package com.ebay.nest.io.sede.thrift;
 ...
 * 
 * The data format: NULL: a single byte \0 NON-NULL Primitives: ALWAYS prepend a single byte \1, and then: Boolean:
 * FALSE = \1, TRUE = \2 Byte: flip the sign-bit to make sure negative comes before positive Short: flip the sign-bit to
 * make sure negative comes before positive Int: flip the sign-bit to make sure negative comes before positive Long:
 * flip the sign-bit to make sure negative comes before positive Double: flip the sign-bit for positive double, and all
 * bits for negative double values String: NULL-terminated UTF-8 string, with NULL escaped to \1 \1, and \1 escaped to
 * \1 \2 NON-NULL Complex Types: Struct: first the single byte \1, and then one field by one field. List: size stored as
 * Int (see above), then one element by one element. Map: size stored as Int (see above), then one key by one value, and
 * then the next pair and so on. Binary: size stored as Int (see above), then the binary data in its original form
 * 
 * Note that the relative order of list/map/binary will be based on the size first (and elements one by one if the sizes
 * are equal).
 * 
 * This protocol takes an additional parameter SERIALIZATION_SORT_ORDER which is a string containing only "+" and "-".
 * The length of the string should equal to the number of fields in the top-level struct for serialization. "+" means
 * the field should be sorted ascendingly, and "-" means descendingly. The sub fields in the same top-level field will
 * have the same sort order.
 * 
 * This is not thrift compliant in that it doesn't write out field ids so things cannot actually be versioned.
 */
public class TBinarySortableProtocol extends TProtocol implements ConfigurableTProtocol, WriteNullsProtocol,
		WriteTextProtocol {

	static final Log LOG = LogFactory.getLog(TBinarySortableProtocol.class.getName());
	...
	/**
	 * The stack level of the current field. Top-level fields have a stackLevel value of 1. Each nested struct/list/map
	 * will increase the stackLevel value by 1.
	 */
	int stackLevel;
	/**
	 * The field ID in the top level struct. This is used to determine whether this field should be sorted ascendingly
	 * or descendingly.
	 */
	int topLevelStructFieldID;
	/**
	 * A string that consists of only "+" and "-". It should have the same length as the number of fields in the top
	 * level struct. "+" means the corresponding field is sorted ascendingly and "-" means the corresponding field is
	 * sorted descendingly.
	 */
	String sortOrder;
	/**
	 * Whether the current field is sorted ascendingly. Always equals to sortOrder.charAt(topLevelStructFieldID) != '-'
	 */
	boolean ascending;
	...
	/**
	 * This method always return the same instance of TMap to avoid creating new instances. It is the responsibility of
	 * the caller to read the value before calling this method again.
	 */
	@Override
	...
	/**
	 * This method always return the same instance of TList to avoid creating new instances. It is the responsibility of
	 * the caller to read the value before calling this method again.
	 */
	@Override
	...
	/**
	 * This method always return the same instance of TSet to avoid creating new instances. It is the responsibility of
	 * the caller to read the value before calling this method again.
	 */
	@Override
	...
	public int readI32() throws TException {
		...
		return (((i32rd[0] ^ 0x80) & 0xff) << 24) | ((i32rd[1] & 0xff) << 16) | ((i32rd[2] & 0xff) << 8)
				| ((i32rd[3] & 0xff));
	}
	...
	public long readI64() throws TException {
		...
		return ((long) ((i64rd[0] ^ 0x80) & 0xff) << 56) | ((long) (i64rd[1] & 0xff) << 48)
				| ((long) (i64rd[2] & 0xff) << 40) | ((long) (i64rd[3] & 0xff) << 32)
				| ((long) (i64rd[4] & 0xff) << 24) | ((long) (i64rd[5] & 0xff) << 16) | ((long) (i64rd[6] & 0xff) << 8)
				| ((i64rd[7] & 0xff));
	}
	...
	public double readDouble() throws TException {
		...
		if ((i64rd[0] & 0x80) != 0) {
			// Positive number
			v = ((long) ((i64rd[0] ^ 0x80) & 0xff) << 56) | ((long) (i64rd[1] & 0xff) << 48)
					| ((long) (i64rd[2] & 0xff) << 40) | ((long) (i64rd[3] & 0xff) << 32)
					| ((long) (i64rd[4] & 0xff) << 24) | ((long) (i64rd[5] & 0xff) << 16)
					| ((long) (i64rd[6] & 0xff) << 8) | ((i64rd[7] & 0xff));
		} else {
			// Negative number
			v = ((long) ((i64rd[0] ^ 0xff) & 0xff) << 56) | ((long) ((i64rd[1] ^ 0xff) & 0xff) << 48)
					| ((long) ((i64rd[2] ^ 0xff) & 0xff) << 40) | ((long) ((i64rd[3] ^ 0xff) & 0xff) << 32)
					| ((long) ((i64rd[4] ^ 0xff) & 0xff) << 24) | ((long) ((i64rd[5] ^ 0xff) & 0xff) << 16)
					| ((long) ((i64rd[6] ^ 0xff) & 0xff) << 8) | (((i64rd[7] ^ 0xff) & 0xff));
		}
