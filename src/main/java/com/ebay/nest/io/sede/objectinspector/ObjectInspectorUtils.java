...
import com.ebay.nest.io.sede.objectinspector.ObjectInspectorFactory.ObjectInspectorOptions;
import com.ebay.nest.io.sede.objectinspector.primitive.AbstractPrimitiveWritableObjectInspector;
import com.ebay.nest.io.sede.objectinspector.primitive.BinaryObjectInspector;
...
import com.ebay.nest.io.sede.objectinspector.primitive.DoubleObjectInspector;
import com.ebay.nest.io.sede.objectinspector.primitive.FloatObjectInspector;
...
import com.ebay.nest.io.sede.objectinspector.primitive.TimestampObjectInspector;
 ...
 * 
 * SerDe classes should call the static functions in this library to create an ObjectInspector to return to the caller
 * of SerDe2.getObjectInspector().
 */
public final class ObjectInspectorUtils {
	 ...
	 * 
	 * DEFAULT means choosing the most efficient way between JAVA and WRITABLE. JAVA means converting all primitive
	 * objects to java primitive objects. WRITABLE means converting all primitive objects to writable objects.
	 * 
	 */
	...
	public static ObjectInspector getStandardObjectInspector(ObjectInspector oi,
		...
		case LIST: {
			...
			result = ObjectInspectorFactory.getStandardListObjectInspector(getStandardObjectInspector(
	...
	/**
	 * Returns a deep copy of the Object o that can be scanned by a StandardObjectInspector returned by
	...
	public static int compare(Object o1, ObjectInspector oi1, Object o2, ObjectInspector oi2,
		...
		case PRIMITIVE: {
			...
			case STRING: {
				if (poi1.preferWritable() || poi2.preferWritable()) {
					...
					return t1 == null ? (t2 == null ? 0 : -1) : (t2 == null ? 1 : ShimLoader.getHadoopShims()
							.compareText(t1, t2));
				} else {
		...
		case STRUCT: {
			...
			for (int i = 0; i < minimum; i++) {
				int r = compare(soi1.getStructFieldData(o1, fields1.get(i)), fields1.get(i).getFieldObjectInspector(),
						soi2.getStructFieldData(o2, fields2.get(i)), fields2.get(i).getFieldObjectInspector(),
						mapEqualComparer);
				if (r != 0) {
					return r;
				}
		...
		case UNION: {
			...
			return compare(uoi1.getField(o1), uoi1.getObjectInspectors().get(tag1), uoi2.getField(o2), uoi2
					.getObjectInspectors().get(tag2), mapEqualComparer);
		}
		default:
	 ...
	 * <li>If the given inspectors are for <code>PRIMITIVE</code> type, the result is the comparison of their type
	 * names.</li>
	 * <li>If the given inspectors are for <code>LIST</code> type, then the result is recursive call to compare the type
	 * of list elements.</li>
	 * <li>If the given inspectors are <code>MAP</code> type, then the result is a recursive call to compare the map key
	 * and value types.</li>
	 * <li>If the given inspectors are <code>STRUCT</code> type, then the result is negative if they do not have the
	 * same number of fields. If they do have the same number of fields, the result is a recursive call to compare each
			...
			return ObjectInspectorFactory
					.getStandardConstantListObjectInspector(
							getStandardObjectInspector(loi.getListElementObjectInspector(),
									ObjectInspectorCopyOption.WRITABLE), (List<?>) writableValue);
		case MAP:
			MapObjectInspector moi = (MapObjectInspector) oi;
			return ObjectInspectorFactory.getStandardConstantMapObjectInspector(
					...
					getStandardObjectInspector(moi.getMapValueObjectInspector(), ObjectInspectorCopyOption.WRITABLE),
					(Map<?, ?>) writableValue);
		default:
