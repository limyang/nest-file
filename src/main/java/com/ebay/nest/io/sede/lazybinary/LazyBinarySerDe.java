...
public class LazyBinarySerDe extends AbstractSerDe {
	...
	/**
	 * Serialize a struct object without writing the byte size. This function is shared by both row serialization and
	 ...
	 * @param warnedOnceNullMapKey
	 *            a boolean indicating whether a warning has been issued once already when encountering null map keys
	 * @return a boolean indicating whether a warning for null map keys has been issued once already
	 */
	private static boolean serializeStruct(Output byteStream, Object obj, StructObjectInspector soi,
			boolean warnedOnceNullMapKey) throws SerDeException {
		// do nothing for null struct
		if (null == obj) {
			return warnedOnceNullMapKey;
		}
		/*
		 * Interleave serializing one null byte and 8 struct fields in each round, in order to support data
		 * deserialization with different table schemas
		 */
		...
		for (int i = 0; i < size; i++) {
			...
			if (7 == i % 8 || i == size - 1) {
				...
				for (int j = lasti; j <= i; j++) {
					warnedOnceNullMapKey = serialize(byteStream, soi.getStructFieldData(obj, fields.get(j)), fields
							.get(j).getFieldObjectInspector(), false, warnedOnceNullMapKey);
				}
				lasti = i + 1;
				nullByte = 0;
			}
	 ...
	 * @param skipLengthPrefix
	 *            a boolean indicating whether length prefix is needed for list/map/struct
	 * @param warnedOnceNullMapKey
	 *            a boolean indicating whether a warning has been issued once already when encountering null map keys
	 * @return a boolean indicating whether a warning for null map keys has been issued once already
	 */
	public static boolean serialize(Output byteStream, Object obj, ObjectInspector objInspector,
		...
		case LIST: {
			...
			for (int eid = 0; eid < size; eid++) {
				warnedOnceNullMapKey = serialize(byteStream, loi.getListElement(obj, eid), eoi, false,
