...
import com.ebay.nest.io.sede.lazybinary.objectinspector.LazyBinaryObjectInspectorFactory;
...
public final class LazyBinaryUtils {
	...
	/**
	 * Record is the unit that data is serialized in. A record includes two parts. The first part stores the size of the
	 * element and the second part stores the real element. size element record -> |----|-------------------------|
	 * 
	 * A RecordInfo stores two information of a record, the size of the "size" part which is the element offset and the
	 * size of the element part which is element size.
	 */
	 ...
	 * 
	 * For void, boolean, byte, short, int, long, float and double, there is no offset and the size is fixed. For
	 * string, map, list, struct, the first four bytes are used to store the size. So the offset is 4 and the size is
	 * computed by concating the first four bytes together. The first four bytes are defined with respect to the offset
	 * in the bytes arrays. For timestamp, if the first bit is 0, the record length is 4, otherwise a VInt begins at the
	 * 5th byte and its length is added to 4.
	 * 
	 ...
	 *            the byte array
	 * @param offset
	 *            the offset in the byte array where the VLong is stored
	 * @return the long
	 */
	...
	/**
	 * Returns the lazy binary object inspector that can be used to inspect an lazy binary object of that typeInfo
	 * 
		...
		if (result == null) {
			...
			case MAP: {
				...
				ObjectInspector valueObjectInspector = getLazyBinaryObjectInspectorFromTypeInfo(mapTypeInfo
						.getMapValueTypeInfo());
				result = LazyBinaryObjectInspectorFactory.getLazyBinaryMapObjectInspector(keyObjectInspector,
			...
			case STRUCT: {
				...
				result = LazyBinaryObjectInspectorFactory.getLazyBinaryStructObjectInspector(fieldNames,
