...
public class TimestampWritable implements WritableComparable<TimestampWritable> {
	...
	private static final ThreadLocal<DateFormat> threadLocalDateFormat = new ThreadLocal<DateFormat>() {
		@Override
	...
	/**
	 * true if data is stored in timestamp field rather than byte arrays. allows for lazy conversion to bytes when
	...
	/**
	 * @return length of serialized TimestampWritable data. As a side effect, populates the internal byte array if
	 *         empty.
	 */
	int getTotalLength() {
		checkBytes();
	...
	/**
	 * @return byte[] representation of TimestampWritable that is binary sortable (7 bytes for seconds, 4 bytes for
	 *         nanoseconds)
	 */
	...
	public void setBinarySortable(byte[] bytes, int binSortOffset) {
		...
		if (hasSecondVInt) {
			LazyBinaryUtils.writeVLongToByteArray(internalBytes, 4 + WritableUtils.decodeVIntSize(internalBytes[4]),
					seconds >> 31);
		}
	...
	/**
	 * The data of TimestampWritable can be stored either in a byte[] or in a Timestamp object. Calling this method
	 * ensures that the byte[] is populated from the Timestamp object if previously empty.
	 */
	...
	public void readFields(DataInput in) throws IOException {
		...
		if (TimestampWritable.hasDecimalOrSecondVInt(internalBytes[0])) {
			...
			if (vlong < -1000000000 || vlong > 999999999) {
				throw new IOException("Invalid first vint value (encoded nanoseconds) of a TimestampWritable: " + vlong
	 ...
	 * @param offset
	 *            destination offset in the byte array
	 */
	public static void convertTimestampToBytes(Timestamp t, byte[] b, int offset) {
		...
		if (hasSecondVInt) {
			LazyBinaryUtils.writeVLongToByteArray(b, offset + 4 + WritableUtils.decodeVIntSize(b[offset + 4]),
					seconds >> 31);
		}
	}

	/**
	 * Given an integer representing nanoseconds, write its serialized value to the byte array b at offset
	 * 
	 * @param nanos
	 * @param b
	 * @param offset
	...
	/**
	 * Writes <code>value</code> into <code>dest</code> at <code>offset</code> as a seven-byte serialized long number.
	 */
	 ...
	 * @param bytes
	 * @param offset
	 * @return integer represented by the four bytes in <code>bytes</code> beginning at <code>offset</code>
	 */
