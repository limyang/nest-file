...
public final class TypeInfoUtils {
	...
	/**
	 * Return the extended TypeInfo from a Java type. By extended TypeInfo, we allow unknownType for java.lang.Object.
	 * 
	 * @param t
	 *            The Java type.
	 * @param m
	...
	private static TypeInfo getExtendedTypeInfoFromJavaType(Type t, Method m) {
		...
		if (t instanceof ParameterizedType) {
			...
			if (List.class == (Class<?>) pt.getRawType() || ArrayList.class == (Class<?>) pt.getRawType()) {
				return TypeInfoFactory.getListTypeInfo(getExtendedTypeInfoFromJavaType(pt.getActualTypeArguments()[0],
						m));
			}
			// Map?
	 ...
	 * 
	 * @param size
	 *            In case the last parameter of Method is an array, we will try to return a List<TypeInfo> with the
	 *            specified size by repeating the element of the array at the end. In case the size is smaller than the
	 *            minimum possible number of arguments for the method, null will be returned.
	 */
	public static List<TypeInfo> getParameterTypeInfos(Method m, int size) {
		...
		Type lastParaElementType = TypeInfoUtils.getArrayElementType(methodParameterTypes.length == 0 ? null
	...
	/**
	 * Parse a recursive TypeInfo list String. For example, the following inputs are valid inputs:
	 * "int,string,map<string,int>,list<map<int,list<string>>>,list<struct<a:int,b:string>>" The separators between
	 * TypeInfos can be ",", ":", or ";".
	 * 
	 * In order to use this class: TypeInfoParser parser = new TypeInfoParser("int,string"); ArrayList<TypeInfo>
	 * typeInfos = parser.parseTypeInfos();
	 */
	private static class TypeInfoParser {
		...
		/**
		 * Tokenize the typeInfoString. The rule is simple: all consecutive alphadigits and '_', '.' are in one token,
		 * and all other characters are one character per token.
		 * 
		...
		private static ArrayList<Token> tokenize(String typeInfoString) {
			...
			while (end <= typeInfoString.length()) {
				...
				if (end == typeInfoString.length() || !isTypeChar(typeInfoString.charAt(end - 1))
		...
		private Token expect(String item, String alternative) {
			...
			if (item.equals("type")) {
				if (!serdeConstants.LIST_TYPE_NAME.equals(t.text) && !serdeConstants.MAP_TYPE_NAME.equals(t.text)
					...
					throw new IllegalArgumentException("Error: " + item + " expected at the position " + t.position
							+ " of '" + typeInfoString + "' but '" + t.text + "' is found.");
				}
	...
	static Map<TypeInfo, ObjectInspector> cachedStandardObjectInspector = new ConcurrentHashMap<TypeInfo, ObjectInspector>();

	/**
	 * Returns the standard object inspector that can be used to translate an object of that typeInfo to a standard
	 * object type.
	 */
	...
	static Map<TypeInfo, ObjectInspector> cachedStandardJavaObjectInspector = new ConcurrentHashMap<TypeInfo, ObjectInspector>();
	...
	public static TypeInfo getTypeInfoFromObjectInspector(ObjectInspector oi) {
		...
		case LIST: {
			...
			result = TypeInfoFactory
					.getListTypeInfo(getTypeInfoFromObjectInspector(loi.getListElementObjectInspector()));
			break;
		}
		case MAP: {
			MapObjectInspector moi = (MapObjectInspector) oi;
			result = TypeInfoFactory.getMapTypeInfo(getTypeInfoFromObjectInspector(moi.getMapKeyObjectInspector()),
