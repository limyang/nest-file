...
public interface HadoopShims {
	 ...
	 */
	String getTaskAttemptLogUrl(JobConf conf, String taskTrackerHttpAddress, String taskAttemptId)
	 ...
	 */
	public MiniMrShim getMiniMrCluster(Configuration conf, int numberOfTaskTrackers, String nameNode, int numDir)
			throws IOException;
	...
	/**
	 * Returns a shim to wrap MiniDFSCluster. This is necessary since this class was moved from org.apache.hadoop.dfs to
	 ...
	 * 
	 * Hive binary that compiled Text.compareTo(Text) with hadoop 0.20 won't work with hadoop 0.17 because in hadoop
	 * 0.20, Text.compareTo(Text) is implemented in org.apache.hadoop.io.BinaryComparable, and Java compiler references
	...
	/**
	 * getTaskJobIDs returns an array of String with two elements. The first element is a string representing the task
	 * id and the second is a string representing the job id. This is necessary as TaskID and TaskAttemptID are not
	 * supported in Haddop 0.17
	 */
	...
	/**
	 * Hive uses side effect files exclusively for it's output. It also manages the setup/cleanup/commit of output from
	 * the hive client. As a result it does not need support for the same inside the MR framework
	 * 
	 * This routine sets the appropriate options related to bypass setup/cleanup/commit support in the MR framework, but
	 * does not set the OutputFormat class.
	 */
	 ...
	 * 
	 * In secure versions of Hadoop, this simply returns the current access control context's user, ignoring the
	 * configuration.
	 */
	 ...
	 */
	public <T> T doAs(UserGroupInformation ugi, PrivilegedExceptionAction<T> pvea) throws IOException,
			InterruptedException;

	/**
	 * Once a delegation token is stored in a file, the location is specified for a child process that runs hadoop
	 * operations, using an environment variable .
	 * 
	 * @return Return the name of environment variable used by hadoop to find location of token file
	 */
	public String getTokenFileLocEnvName();

	/**
	 * Get delegation token from filesystem and write the token along with metastore tokens into a file
	 * 
	 * @param conf
	 ...
	 * 
	 * In secure versions of Hadoop, this returns the short name (after undergoing the translation in the kerberos name
	 * rule mapping). In unsecure versions of Hadoop, this returns the name of the subject
	 */
	...
	/**
	 * Get the string form of the token given a token signature. The signature is used as the value of the "service"
	 * field in the token for lookup. Ref: AbstractDelegationTokenSelector in Hadoop. If there exists such a token in
	 * the token cache (credential store) of the job, the lookup returns that. This is relevant only when running
	 * against a "secure" hadoop release The method gets hold of the tokens if they are set up by hadoop - this should
	 * happen on the map/reduce tasks if the client added the tokens into hadoop's credential store in the front end
	 * during job submission. The method will select the hive delegation token among the set of tokens and return the
	 * string form of it
	 * 
	...
	/**
	 * Convert the ClusterStatus to its Thrift equivalent: JobTrackerState. See MAPREDUCE-2455 for why this is a part of
	 * the shim.
	 * 
	 * @param clusterStatus
	 * @return the matching JobTrackerState
	 * @throws Exception
	 *             if no equivalent JobTrackerState exists
	 */
	...
	/**
	 * Move the directory/file to trash. In case of the symlinks or mount points, the file is moved to the trashbin in
	 * the actual volume of the path p being deleted
	 * 
	 * @param fs
	 * @param path
	 * @param conf
	...
	/**
	 * Get the default block size for the path. FileSystem alone is not sufficient to determine the same, as in case of
	 * CSMT the underlying file system determines that.
	 * 
	 * @param fs
	 * @param path
	 * @return
	...
	/**
	 * Get the default replication for a path. In case of CSMT the given path will be used to locate the actual
	 * filesystem.
	 * 
	 * @param fs
	 * @param path
	...
	public interface HCatHadoopShims {
		...
		enum PropertyName {
			CACHE_ARCHIVES, CACHE_FILES, CACHE_SYMLINK
		}
